...
io.on("connection", (socket) => {
    ...
    socket.on("sendMessage", async (data) => {
        try {
            ...
            // Replace restricted words or words containing digits with "X"
            let sanitizedMessage = data.message.split(" ").map(word => {
                const lowerWord = word.toLowerCase();
                const containsDigit = /\d/.test(word); // checks if word contains any digit
                if (restrictedList.includes(lowerWord) || containsDigit) {
                    return "X";
                }
                return word;
            }).join(" ");
