...
router.get("/delete-account-info", async(req, res)=>{
  try {

    res.render("delete-account-info");
...
if (existingRequest) {
  srequest = existingRequest;
}

res.render("support.ejs", { srequest , messagesToday, messages});
...
router.post(
  ...
  async (req, res) => {
    ...
    await SuccessStoryRequest.deleteOne({ user: userId });
...
router.post("/eddit-profile", async(req, res)=>{
  try {
    ...
    req.flash("error", "New details saved!.");
...
router.get("/download-receipt", async (req, res) => {
  ...
  try {
    ...
    if (order.order_status === "PAID") {
      ...
      pdf.moveDown(0.5);
      pdf.text("For any queries, contact support@indianwife.in", {
        align: "center",
      });
...
router.post("/payment", async (req, res) => {
  try {
    ...
    console.log("Payment Request Data:", { name, email, phone, amount, plan, promocode });

    savePromoCodeUsage(promocode, userId, plan);

    if (!amount || Number(amount) < 1) {
      return res.send("Invalid amount. Must be ₹1 or more.");
    }
    ...
    const orderData = {
      ...
      order_note: plan,
      order_meta: {
        return_url: `https://indianwife.in/payment/status?order_id={order_id}`,
        notify_url: `https://indianwife.in/payment/webhook`,
        promo_code: promocode
      }
    };

    console.log("Sending orderData to Cashfree:", orderData);

    const response = await fetch(process.env.CASHFREE_BASE_URL || "https://api.cashfree.com/pg/orders", {
      method: "POST",
      headers: {
        "x-client-id": process.env.CASHFREE_CLIENT_ID,
        "x-client-secret": process.env.CASHFREE_CLIENT_SECRET,
    ...
    if (data.payment_link) {
      ...
      return res.send(`Error creating payment link: ${data.message || "Unknown error"}`);
    }
...
router.post("/intrest", async (req, res) => {
  try {
      ...
      if (!currUser.plan || currUser.plan.intrests <= 0) {
        return res.json({ success: false, message: "UpgradeMembership" });
    }
      ...
      if (index === -1) {
          ...
          currUser.plan.intrests -= 1;
          await user.save();
          await currUser.save();
          req.flash("error", "Interest added.");
          return res.json({ success: true, action: "added", message: "Interest added" });
      } else {
          // Remove interest if already present
          user.superInterests.splice(index, 1);
          await user.save();
          req.flash("error", "Interest removed");
          return res.json({ success: true, action: "removed", message: "Interest removed" });
      }
  } catch (error) {
      console.error("Error:", error);
